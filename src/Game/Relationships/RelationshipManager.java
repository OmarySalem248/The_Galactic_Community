...
import Game.Actions.Interactions.*;
        ...
        for (Colonist c1 : colonists) {
            ...
            for (Colonist c2 : candidates) {
                ...
                if (rel1 != null && rel2 != null) {
                    ...
                    if (rel1.getValue(SEXUAL) >= 50 && rel2.getValue(SEXUAL) >= 50) {
                        intercourse.execute(c1, c2, rel1, rel2);
                    }
                    if (rel1.getValue(ROMANTIC) >= 50 && rel2.getValue(ROMANTIC) >= 50) {
                        date.execute(c1, c2, rel1, rel2);
                    }
                    if (rel1.getValue(ROMANTIC) >= 50 && rel2.getValue(ROMANTIC) >= 50 && !c1.isEngagedTo(c2)) {
